#!/usr/bin/env bash
# Launch or attach to a MusicGen BED generation run on the UM880 box over SSH — the
# survivable-tmux runner for the background-music beds. Two kinds:
#   brand  -> generate_brand_beds.py  (per-vibe beds + brand->bed map for the website)
#   mood   -> generate_beds.py        (per-mood beds for content audio)
#
# GPU by default (MusicGen on the 780M via ROCm; frees the display during the run since
# the iGPU drives the screen). --cpu runs on the host CPU (slow for MusicGen) and keeps
# the desktop up. Everything after <host> <kind> passes to the generator.
#
# Usage
# -----
#   scripts/05f-gen-beds-remote.sh <host> brand --model musicgen-medium --duration 20
#   scripts/05f-gen-beds-remote.sh <host> mood  --per-mood 2 --duration 15
#   scripts/05f-gen-beds-remote.sh <host> brand --cpu            # CPU, desktop stays up
#
#   scripts/05f-gen-beds-remote.sh <host> --attach
#   scripts/05f-gen-beds-remote.sh <host> --status [--tail 40]
#   scripts/05f-gen-beds-remote.sh <host> --stop
#
# First-run downloads a MusicGen model (small ~2GB / medium ~15GB) — pre-fetch with
# 02-download-models.sh to avoid downloading while the display is isolated.
#
# Overrides: REMOTE_USER (panamorphic), REMOTE_DIR (Developer/Vishru/brand-media-gen),
#            TMUX_SESSION (bedsgen).

set -euo pipefail

REMOTE_USER="${REMOTE_USER:-panamorphic}"
REMOTE_DIR="${REMOTE_DIR:-Developer/Vishru/brand-media-gen}"
TMUX_SESSION="${TMUX_SESSION:-bedsgen}"

if [[ $# -lt 1 ]]; then
    echo "Usage: $0 <[user@]host> <brand|mood> [--cpu] [gen args...]" >&2
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        brand)     KIND="brand"; SCRIPT="scripts/generate_brand_beds.py"; shift ;;
        mood)      KIND="mood";  SCRIPT="scripts/generate_beds.py"; shift ;;
...
# ── Helpers (same contract as 05/05b/05c/05d/05e) ──────────────────────────────
...
check_gpu_sudo() {
    ...
    echo "       up, or run on CPU with --cpu (desktop stays up; MusicGen on CPU is slow)." >&2
    echo "" >&2
    print_sudoers_setup
    exit 1
}

newest_log() { ssh_run "ls -t ~/${REMOTE_DIR}/output/audio/beds-run-*.log 2>/dev/null | head -1"; }

# ── Status mode ────────────────────────────────────────────────────────────────
if [[ "$MODE" == "status" ]]; then
    check_reachable
    ALIVE=$(ssh_run "tmux has-session -t '${TMUX_SESSION}' 2>/dev/null && echo yes || echo no")
    echo "=== beds status on ${HOST} ==="
    ...
    echo ""
    echo "Brand beds:"; ssh_run "ls ~/${REMOTE_DIR}/output/audio/brand-beds/*.wav 2>/dev/null | wc -l | sed 's/^/  /' ; ls ~/${REMOTE_DIR}/output/audio/brand-beds/map.json 2>/dev/null | sed 's#.*/#  map: #' || true"
    echo "Mood beds :"; ssh_run "ls ~/${REMOTE_DIR}/output/audio/beds/*/*.wav 2>/dev/null | wc -l | sed 's/^/  /' || echo '  (none)'"
    exit 0
fi
...
if [[ "$MODE" == "stop" ]]; then
    ...
    ALIVE=$(ssh_run "tmux has-session -t '${TMUX_SESSION}' 2>/dev/null && echo yes || echo no")
    if [[ "$ALIVE" == "yes" ]]; then
        echo "Interrupting beds run in tmux '${TMUX_SESSION}' on ${HOST}..."
...
# ── Run mode ───────────────────────────────────────────────────────────────────
if [[ -z "$KIND" ]]; then
    echo "ERROR: specify which beds — 'brand' or 'mood'." >&2
    echo "  $0 ${HOST_ARG} brand --model musicgen-medium --duration 20" >&2
    echo "  $0 ${HOST_ARG} mood  --per-mood 2" >&2
    exit 1
fi
...
if [[ "$DEVICE_MODE" == "gpu" ]]; then
    ...
    ENV_SETUP=""
    DISPLAY_FREE="echo '[beds] Freeing display engine — desktop goes DOWN during this GPU run...'
sudo systemctl isolate multi-user.target"
    DISPLAY_TRAP="trap 'sudo systemctl isolate graphical.target || true' INT TERM HUP EXIT"
    DISPLAY_RESTORE="echo '[beds] Restoring desktop...'
sudo systemctl isolate graphical.target || true
trap - INT TERM HUP EXIT"
else
    RUN_INVOCATION="python ${SCRIPT} --cpu${QUOTED_ARGS}"
...
${ENV_SETUP}
LOG=\"output/audio/beds-run-\$(date +%Y%m%d-%H%M%S).log\"
${DISPLAY_TRAP}
${DISPLAY_FREE}

echo \"=== gen-beds-remote: ${KIND} beds on ${DEVICE_MODE} ===\" | tee -a \"\$LOG\"
...
if [[ \$EXIT_CODE -eq 0 ]]; then
    echo '✓ Beds complete. See output/audio/${KIND}-beds/ (brand) or output/audio/beds/ (mood).'
else
    echo \"✗ Beds exited with code \$EXIT_CODE (see \$LOG)\"
fi
echo 'Session will stay open. Detach with Ctrl-B D, or type exit to close.'
bash
"

echo "=== gen-beds-remote ==="
echo "Host    : ${HOST}"
echo "Session : ${TMUX_SESSION}"
echo "Kind    : ${KIND} beds  ($SCRIPT)"
echo "Device  : ${DEVICE_MODE}$([[ "$DEVICE_MODE" == gpu ]] && echo '  (desktop freed during run)' || echo '  (CPU, desktop stays up — MusicGen slow)')"
...
echo ""
echo "✓ Beds run started in tmux session '${TMUX_SESSION}'"
echo ""
